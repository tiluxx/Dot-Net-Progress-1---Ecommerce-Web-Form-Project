    ...
    public class PDFGenerator
    {
        public ActionResult CreatePDF(Order order, List<OrderDetail> orderDetails)
        {
            using (PdfDocument document = new PdfDocument())
            {
                ...
                element = new PdfTextElement($"Order ID: {order.OrderID}", timesRoman)
                {
                    Brush = new PdfSolidBrush(new PdfColor(126, 155, 203))
                };
                result = element.Draw(page, new PointF(10, result.Bounds.Bottom + 25));
                element = new PdfTextElement($"Order Date: {order.OrderDate.ToString("MMMM dd yyyy, hh:mm: tt", CultureInfo.InvariantCulture)}", timesRoman)
                {
                    Brush = new PdfSolidBrush(new PdfColor(126, 155, 203))
                };
                result = element.Draw(page, new PointF(10, result.Bounds.Bottom + 25));
                ...
                fileStreamResult.FileDownloadName = $"Order_Report_{order.OrderID}_{DateTime.Now.ToString("ddMMMMyyyyHHmmssfffff")}.pdf";
                document.Close(true);
